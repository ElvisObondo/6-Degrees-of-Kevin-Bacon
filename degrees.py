...
def load_data(directory):
    ...
    # Load movies
    with open("large/movies.csv", encoding="utf-8") as f:
        reader = csv.DictReader(f)
        for row in reader:
            movies[row["id"]] = {
                "title": row["title"],
    ...
    # Load stars
    with open("large/stars.csv", encoding="utf-8") as f:
        reader = csv.DictReader(f)
        for row in reader:
            try:
...
def main():
    if len(sys.argv) > 2:
        sys.exit("Usage: python degrees.py large")
    ...
    while not frontier.empty():
        ...
        for movie_id, person_id in neighbors_for_person(current_state):
            if not frontier.contains_state(person_id) and person_id not in explored:
                child_node = Node(state=person_id,
                                  parent=current_node, action=movie_id)
